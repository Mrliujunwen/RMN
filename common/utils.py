...
def parse_args(arg_mode):
    ...
    parser.add_argument('-val', type=str, default="test", help='batch size for validation')
    ''' about SCR '''
    ...
    ''' about MR-Net (Mamba Relation Network) '''
    parser.add_argument('-model_type', type=str, default='renet', choices=['renet', 'mrnet'],
                        help='model type: renet (original) or mrnet (Mamba Relation Network)')
    parser.add_argument('-model_version', type=str, default='v1', choices=['v1', 'v2', 'v3', 'v4', 'v5'],
                        help='MR-Net version: v1 (1D Mamba), v2 (SS2D + 1D Mamba), v3 (Full Mamba), v4 (Multi-Expert SCR + Fixed), v5 (Multi-Expert SCR + Soft Routing)')
    parser.add_argument('-mamba_hidden_dim', type=int, default=256,
                        help='hidden dimension for Mamba relation reasoner')
    ...
    parser.add_argument('-mamba_n_layers', type=int, default=2,
                        help='number of 1D Mamba layers for relation reasoning')
    parser.add_argument('-ss2d_n_layers', type=int, default=2,
                        help='number of SS2D layers for spatial feature processing')
    parser.add_argument('-mamba_dropout', type=float, default=0.1,
                        help='dropout rate for Mamba layers')
    
    ''' about Cluster-Guided Mamba '''
    parser.add_argument('-use_cluster', action='store_true',
                        default=True,
                        help='use cluster-guided Mamba scanning (聚类引导的Mamba扫描)')
    parser.add_argument('-n_clusters', type=int, default=5,
                        help='number of clusters for cluster-guided Mamba (聚类数量)')
    
    ''' about Fusion-Net '''
    parser.add_argument('-fusion_method', type=str, default='adaptive',
                        choices=['adaptive', 'concat', 'add'],
                        help='fusion method for CCA+Mamba: adaptive, concat, add')
    
    ''' about Multi-Expert SCR (V4/V5) '''
    parser.add_argument('-num_experts', type=int, default=3,
                        help='number of SCR experts for V4/V5 (multi-scale kernels)')
    parser.add_argument('-router_hidden', type=int, default=128,
                        help='hidden dimension for soft routing network in V5')
    parser.add_argument('-cross_kernel', type=int, default=5,
                        help='kernel size for cross-correlation computation in V3/V4/V5')

    ''' about env '''
    parser.add_argument('-gpu', default='2', help='the GPU ids e.g. \"0\", \"0,1\", \"0,1,2\", etc')
    parser.add_argument('-extra_dir', type=str, default='test256', help='extra dir name added to checkpoint dir')
